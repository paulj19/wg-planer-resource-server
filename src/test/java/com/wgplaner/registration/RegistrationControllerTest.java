...
public class RegistrationControllerTest extends BaseIT {
  ...
  public void setup() {
    ...
    doReturn(1L).when(authServerRequester).registerUserAndFetchOid(any(), any());
  }
  ...
  public void whenPostWithValidData_shouldCreateAndRespondWithNewCreatedUser() throws Exception {
    ...
    ResultActions resultActions = mockMvc.perform(MockMvcRequestBuilders.post("/register/new")
        .contentType(MediaType.APPLICATION_JSON).content(JsonUtils.toJsonString(registrationDto)));
  ...
  public void whenPostWithInValidData_shouldNotCreateUserAndRespondWith422(RegistrationDto invalidRegistrationDto,
  ...
  private static Stream<Arguments> data() {
    ...
    return Stream.of(
        // username < 5
        Arguments.of(
            new RegistrationDto("a", "foo@email.com", "Password123!", null, AuthServer.HOME_BREW),
            List.of(
                "username must be between 4 and 32 chars, only letters numbers and . with a word prefix and suffix")),
        // username > 32
        Arguments.of(
            new RegistrationDto("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "foo@email.com", "Password123!", null,
                AuthServer.HOME_BREW),
            List.of(
                "username must be between 4 and 32 chars, only letters numbers and . with a word prefix and suffix")),
        // username null
        Arguments.of(
            new RegistrationDto(null, "foo@email.com", "Password123!", null, AuthServer.HOME_BREW),
            List.of(
                "username must be between 4 and 32 chars, only letters numbers and . with a word prefix and suffix")),
        // username empty
        Arguments.of(
            new RegistrationDto("", "foo@email.com", "Password123!", null, AuthServer.HOME_BREW),
            List.of(
                "username must be between 4 and 32 chars, only letters numbers and . with a word prefix and suffix")),
        // invalid email
        Arguments.of(
            new RegistrationDto("username", "fooemail.com", "Password123!", null, AuthServer.HOME_BREW),
            List.of("Invalid email")),
        // email null
        Arguments.of(
            new RegistrationDto("username", null, "Password123!", null, AuthServer.HOME_BREW),
            List.of("Invalid email")),
        // email empty
        Arguments.of(
            new RegistrationDto("username", "", "Password123!", null, AuthServer.HOME_BREW),
            List.of("Invalid email")),
        ...
        // pw no capital
        Arguments.of(
            new RegistrationDto("username", "foo@email.com", "password123!", null, AuthServer.HOME_BREW),
            List.of(pwErrorExpectedMessage)),
        // pw no numerical
        Arguments.of(
            new RegistrationDto("username", "foo@email.com", "Password!", null, AuthServer.HOME_BREW),
            List.of(pwErrorExpectedMessage)),
        // pw no special chars
        Arguments.of(
            new RegistrationDto("username", "foo@email.com", "Password123", null, AuthServer.HOME_BREW),
        ...
        // pw null
        Arguments.of(
            new RegistrationDto("username", "foo@email.com", null, null, AuthServer.HOME_BREW),
            List.of(pwErrorExpectedMessage)),
        // pw empty
        Arguments.of(
            new RegistrationDto("username", "foo@email.com", "", null, AuthServer.HOME_BREW),
            List.of(pwErrorExpectedMessage)),
        // username > 32 && invalid email && pw no numerical
        Arguments.of(
            new RegistrationDto("a", "fooemail.com", "Password!", null, AuthServer.HOME_BREW),
            List.of("username must be between 4 and 32 chars, only letters numbers and . with a word prefix and suffix",
                "Invalid email", pwErrorExpectedMessage)),
        // pw empty
        Arguments.of(
            new RegistrationDto("username", "foo@email.com", "Password123!", null, null),
  ...
  public void whenPostWithInValidUserName_shouldNotCreateUserAndRespondWith422(String username) throws Exception {
    RegistrationDto registrationDto = new RegistrationDto(username, "foo@email.com", "Password!", null,
        AuthServer.HOME_BREW);
    // when
    ResultActions resultActions = mockMvc.perform(MockMvcRequestBuilders.post("/register/new")
        .contentType(MediaType.APPLICATION_JSON)
  ...
  public void whenPostWithNonUniqueUsername_shouldNotCreateUserAndRespondWith422() throws Exception {
    RegistrationDto registrationDto = new RegistrationDto("user123", "foo@email.com", "Password123!", null,
        AuthServer.HOME_BREW);
    // pre-condition when
    ResultActions resultActions = mockMvc.perform(MockMvcRequestBuilders.post("/register/new")
        .contentType(MediaType.APPLICATION_JSON)
    ...
    registrationDto = new RegistrationDto("user123", "bar@email.com", "Berlin123!", null, AuthServer.HOME_BREW);
    // when
  ...
  public void whenPostWithNonUniqueEmail_shouldNotCreateUserAndRespondWith422() throws Exception {
    RegistrationDto registrationDto = new RegistrationDto("userABC", "foo@email.com", "Password123!", null,
        AuthServer.HOME_BREW);
    // pre-condition when
    ResultActions resultActions = mockMvc.perform(MockMvcRequestBuilders.post("/register/new")
        .contentType(MediaType.APPLICATION_JSON)
    ...
    resultActions.andExpect(MockMvcResultMatchers.status().isCreated());
    // everything except username different
    registrationDto = new RegistrationDto("user123", "foo@email.com", "Berlin123!", null, AuthServer.HOME_BREW);
    // when
    resultActions = mockMvc.perform(MockMvcRequestBuilders.post("/register/new")
        .contentType(MediaType.APPLICATION_JSON)
        .content(JsonUtils.toJsonString(registrationDto)));
  ...
  //
  // @Test
  // public void whenGetWithNotRegisteredUsername_shouldRespondAsAvailable()
  // throws Exception {
  // //when
  // ResultActions resultActions =
  // mockMvc.perform(MockMvcRequestBuilders.get("/register/username-available")
  ...
  //
  // @Test
  // public void whenGetWithRegisteredUsername_shouldRespondAsNotAvailable()
  // throws Exception {
  // //when
  // ResultActions resultActions =
  // mockMvc.perform(MockMvcRequestBuilders.get("/register/username-available")
  ...
  // resultActions.andExpect(content().string(containsStringIgnoringCase("true")));
  // resultActions =
  // mockMvc.perform(MockMvcRequestBuilders.post("/register").contentType(MediaType.APPLICATION_JSON).content(asJsonString(registrationDto)));
  // resultActions.andExpect(MockMvcResultMatchers.status().isCreated());
  // //when
  // resultActions =
  // mockMvc.perform(MockMvcRequestBuilders.get("/register/username-available")
  ...
  //
  //// @ParameterizedTest
  //// @ValueSource(strings = {"@abcd", "!abcd", ".abcd", "ab^cd", "a&aaaa",
  // ".abdc", "ab..cd", "abdc."})
  //// public void whenGetWithInvalidUsername_shouldRespondAsInvalid(String
  ...
  // //assert
  //// resultActions.andExpect(MockMvcResultMatchers.status().isUnprocessableEntity());
  //// resultActions.andExpect(content().string(containsString("username must be
  // between 4 and 32 chars, only letters numbers and . with a word prefix and
  // suffix")));
  //// }
  //
  ...
  private RegistrationDto getValidRegistrationDto() {
    return new RegistrationDto("usert", "foo@email.com", "Password123!", 123L, AuthServer.HOME_BREW);
  }
}
